...
sys.path.append("../")
# 导入同事写的工具代码
from utils.FT_datacollator import TrainDataCollator, EvalDataCollator, sft_preprocess_function
from utils.datacollator import preprocess_function
...
def main():
    ...
    config_path = os.path.join(current_script_dir, "finetune_config", args.dataset, f"{args.model_name}.yaml")
    ...
    pretrain_checkpoint = paths_config.get('pretrain_checkpoint', None)
    ...
    # 逻辑：如果指定了 checkpoint 且路径存在，直接加载；否则动态创建
    if pretrain_checkpoint and os.path.exists(pretrain_checkpoint):
        logging.info(f"📥 Loading tokenizer from checkpoint: {pretrain_checkpoint}")
        try:
            # 尝试加载，trust_remote_code=True 防止自定义模型报错
            tokenizer = AutoTokenizer.from_pretrained(pretrain_checkpoint, trust_remote_code=True)
        except Exception as e:
            logging.warning(f"Failed to load tokenizer from {pretrain_checkpoint}: {e}")
            logging.info("Fallback: Creating pure ID tokenizer from scratch...")
            tokenizer = create_pure_id_qwen_tokenizer(output_dir=tokenizer_dir, codeword_nums=codeword_nums)
    else:
        logging.info("✨ Creating pure ID tokenizer from scratch (No checkpoint found)...")
    ...
    # except:
    #     logging.warning("⚠️⚠️⚠️Test file not found, using train set as eval!⚠️⚠️⚠️")
    ...
    # --- 核心分支 ---
    if pretrain_checkpoint and os.path.exists(pretrain_checkpoint):
        logging.info(f"📥 Loading model weights from checkpoint: {pretrain_checkpoint}")
        
        # 这里的 kwargs 会覆盖 checkpoint里 config.json 的对应参数
        # 比如我们想强制 use_cache=False，哪怕 checkpoint 里是 True
        ...
        logging.info("✨ Initializing model from scratch (Random Init)...")
        config = config_class(**config_kwargs)
        model = model_class(config)

    logging.info(f"Model loaded. Parameters: {model.num_parameters() / 1e6:.2f} M.")
    # ==========================================================
    # Trainer 准备
    # ==========================================================
