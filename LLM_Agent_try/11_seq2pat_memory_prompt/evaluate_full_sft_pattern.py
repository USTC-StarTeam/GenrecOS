...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate memory-augmented finetuned Qwen on next-item prediction.")
    parser.add_argument(
        "--config",
        type=str,
...
def main():
    ...
    with open(os.path.join(result_dir, f"{args.output_prefix}_metrics.json"), "w", encoding="utf-8") as f:
        json.dump(metric_values, f, indent=2, ensure_ascii=False)
    with open(os.path.join(result_dir, f"{args.output_prefix}_sample_predictions.json"), "w", encoding="utf-8") as f:
