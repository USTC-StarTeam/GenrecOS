...
def parse_args():
    parser = argparse.ArgumentParser(description="Train retrieval-augmented SFT.")
...
def main():
    ...
    cache_dir = os.path.join(script_dir, paths_cfg["cache_dir"], args.track_name)
    output_dir = os.path.join(script_dir, paths_cfg["output_root"], args.track_name)
    logging_dir = os.path.join(script_dir, paths_cfg["logs_dir"], args.track_name)
    ...
    if args.track_name == "base_init":
        model_path = os.path.join(script_dir, paths_cfg["base_model_path"])
    else:
        model_path = os.path.join(script_dir, paths_cfg["strong_init_model_path"])
    ...
    num_proc = int(train_cfg.get("preprocess_num_proc", 1))
    ...
    tokenized = dataset.map(
        ...
        desc=f"Tokenizing augmented SFT dataset ({args.track_name})",
        load_from_cache_file=True,
    )

    total_train_samples = len(tokenized["train"])
    per_device_bs = int(train_cfg["per_device_train_batch_size"])
    grad_accum = int(train_cfg["gradient_accumulation_steps"])
    steps_per_epoch = math.ceil(total_train_samples / (per_device_bs * grad_accum))
    eval_steps = int(train_cfg.get("eval_steps") or steps_per_epoch)
    save_steps = int(train_cfg.get("save_steps") or eval_steps)
    ...
    trainer = Trainer(
        ...
        callbacks=[
            EarlyStoppingCallback(early_stopping_patience=int(train_cfg.get("early_stopping_patience", 4))),
            GpuStatsCallback(),
        ],
    )
