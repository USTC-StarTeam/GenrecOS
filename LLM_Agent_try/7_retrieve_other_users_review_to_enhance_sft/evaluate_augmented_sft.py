...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate retrieval-augmented SFT.")
    parser.add_argument("--track_name", type=str, choices=["base_init", "strong_init"], required=True)
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional override path for evaluation test jsonl.",
    )
    parser.add_argument(
        "--output_tag",
        type=str,
        default="",
        help="Optional suffix tag appended to output filenames, e.g. common4385.",
    )
    return parser.parse_args()


def pooled_embeddings(model, tokenizer, texts: List[str], batch_size: int, max_length: int, show_progress: bool = True):
    all_embeddings = []
    model.eval()
...
def build_prompts(tokenizer, prompts: List[str]) -> Dict[str, torch.Tensor]:
    ...
    return tokenizer(chat_prompts, return_tensors="pt", padding=True, truncation=True)
...
def main():
    ...
    if args.checkpoint_path:
        ...
        checkpoint_path = os.path.join(script_dir, paths_cfg["output_root"], args.track_name, "best_model")

    titles_path = os.path.join(script_dir, paths_cfg["item_titles_path"])
    test_path = args.test_path if args.test_path else os.path.join(data_dir, "test.jsonl")
    ...
    for start in tqdm(range(0, len(dataset), batch_size), desc=f"Evaluating {args.track_name}"):
            ...
            prediction_rows.append(
                {
                    ...
                    "num_retrieved_reviews": int(batch["num_retrieved_reviews"][row_idx]),
                    "top_5": top_items,
                }
            )
    ...
    metric_values["track_name"] = args.track_name
    metric_values["checkpoint_path"] = checkpoint_path
    metric_values["test_path"] = test_path

    file_prefix = args.track_name
    if args.output_tag:
        file_prefix = f"{file_prefix}_{args.output_tag}"

    with open(os.path.join(result_dir, f"{file_prefix}_evaluation_metrics.json"), "w", encoding="utf-8") as f:
        json.dump(metric_values, f, indent=2, ensure_ascii=False)
    with open(os.path.join(result_dir, f"{file_prefix}_sample_predictions.json"), "w", encoding="utf-8") as f:
