...
from common import format_chat_prompt, load_item_titles, normalize_title


def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate interest-augmented prompting.")
...
def main():
    ...
    checkpoint_path = os.path.join(
        script_dir,
        paths["base_model_path"] if args.track_name == "base_init" else paths["strong_init_model_path"],
    )
    ...
    dataset = load_dataset("json", data_files={"test": args.test_path})["test"]
    ...
    metric_values["test_path"] = args.test_path

    metrics_path = os.path.join(results_dir, f"{args.output_name}_metrics.json")
    preds_path = os.path.join(results_dir, f"{args.output_name}_sample_predictions.json")
